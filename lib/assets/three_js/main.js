...
export function init(ctx, data) {
  ...
  if (type == 'pendulums') {
    pendulums(ctx, data['number']);
  } else {
    simpleCube(ctx);
  }
}

async function pendulums(ctx, number_of_pendulums) {  
  ...
  for (let i = 0; i < number_of_pendulums; i++) {
    const pendulum = await createPendulum(scene, new THREE.Vector3(0, 0, -i * 1.2), 1.2 + i * 0.05, 0.5);
    pendulums.push(pendulum);
  }

  ctx.handleEvent("update_time", (time) => {
    pendulums.forEach(pendulum => pendulum.update(time));
...
function simpleCube(ctx) {
  ...
  ctx.root.innerHTML = `
  <div id='${cubeID}' style='width: 896px; height: 600px;' />
  `
  const scene = new THREE.Scene();
...
async function createGround() {
  ...
  const textureColor = await loadTexture(loader, './images/pendulum/paving_color.jpg');
  const textureRoughness = await loadTexture(loader, './images/pendulum/paving_roughness.jpg');
  const textureNormal = await loadTexture(loader, './images/pendulum/paving_normal.jpg');
  const textureAmbientOcclusion = await loadTexture(loader, './images/pendulum/paving_ambient_occlusion.jpg');
...
class Pendulum {
  ...
  update(totalTime) {
    this.stringMesh.rotation.z = this.amplitude * Math.cos((this.frequency * totalTime) / 1000);
...
async function createBallMesh(scene) {
  ...
  const marbleTextureColor = await loader.loadAsync('./images/pendulum/marble_color.jpg');
  const marbleTextureRoughness = await loader.loadAsync('./images/pendulum/marble_roughness.jpg');
